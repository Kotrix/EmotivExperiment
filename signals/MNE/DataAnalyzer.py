...
class DataAnalyzer:
    ...
    def robust_weighted_averaging_absolute(self, epochs: mne.Epochs):
        for input_signal_index in range(0, 10):
            evokeds = self._to_evoked(RobustWeightedAveraging().absolute(epochs.get_data(), input_signal_index),
                                      epochs.info)
    ...
    def robust_weighted_averaging_quadratic(self, epochs: mne.Epochs):
        for input_signal_index in range(0, 10):
            evokeds = self._to_evoked(RobustWeightedAveraging().quadratic(epochs.get_data(), input_signal_index),
                                     epochs.info)
